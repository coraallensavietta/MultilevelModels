...
Refer to the schools identified in problem 7. For each school, find a 95% prediction interval for the
mathematics exam score of a single student with an SES value of 7. Comment on differences and
similarities between the two analyses and among the five schools.
#prediction
```{r}
require(merTools)
...
```
If I were to consider an alternative model, I would use a random intercepts and slopes model. This would allow each school to start at a different intercept and be impacted by SES differently.